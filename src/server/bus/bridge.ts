...
export function bridgeWorkspaceBus(wsBus: WorkspaceBus, directory: string): () => void {
	...
	wsBus.on("session:error", (data) => handler("session:error", data))
	wsBus.on("session:error-clear", (data) => handler("session:error-clear", data))
	wsBus.on("file:changed", (data) => handler("file:changed", data))
	wsBus.on("git:changed", (data) => handler("git:changed", data))
